...
-- 3. Tabla user_roles (separada por seguridad)
...
-- 4. Función has_role (SECURITY DEFINER para evitar recursión en RLS)
...
CREATE POLICY "Solo admins asignan roles"
ON public.user_roles FOR INSERT
TO authenticated
WITH CHECK (public.has_role(auth.uid(), 'admin'));

CREATE POLICY "Solo admins quitan roles"
ON public.user_roles FOR DELETE
TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

-- 8. Ahora que existe has_role, agregamos políticas de admin a video_channels
CREATE POLICY "Admins gestionan video_channels"
